...
from utils.GPVAE_utils import *
from utils.GPVAE_nn_utils_varia import *
from utils.GPVAE_gp_kernel import *
...
class VAE(tf.keras.Model):
    ...
    def compute_mse(self, x, y=None, m_mask=None, binary=False):
        ...
        # cast
        x_hat_mean = tf.cast(x_hat_mean, tf.float32)
    ...
    def _compute_loss(self, x,y, m_mask=None, return_parts=False):
        assert len(x.shape) == 3, "Input should have shape: [batch_size, time_length, data_dim]"
        x = tf.identity(x)  # in case x is not a Tensor already...
        ...
        mse = self.compute_mse(x,y) # mse instead of nll 
    ...
    def compute_loss(self, x, y, m_mask=None, return_parts=False):
        del m_mask
    ...
    def get_trainable_vars(self):
        self.compute_loss(tf.random.normal(shape=(1, self.time_length, self.data_dim), dtype=tf.float32),
                          tf.random.normal(shape=(1, self.time_length, self.data_dim), dtype=tf.float32),
...
class HI_VAE(VAE):
    ...
    def compute_loss(self, x, y, m_mask=None, return_parts=False):
        return self._compute_loss(x, y, m_mask=m_mask, return_parts=return_parts)
...
class GP_VAE(HI_VAE):
    ...
    def _get_prior(self):
        if self.prior is None:
            ...
            kernel_matrix_tiled = tf.concat(tiled_matrices, axis=0)  ###### Use tf.concat here #########
    ...
    def call(self, inputs):
        """Forward pass through the GP-VAE model.

        Args:
            inputs (Tensor): The input tensor, expected to have shape [batch_size, time_length, data_dim].

        Returns:
            Tensor: The reconstructed output tensor.
        """
        # 입력 텐서의 타입 확인 및 변환
        ...
        batch_size, time_length, data_dim = input_shape[0], input_shape[1], input_shape[2]

        # batch_size나 다른 차원이 None일 경우 처리
        if batch_size is None:
            raise ValueError("Batch size cannot be None.")
        if time_length is None or data_dim is None:
            raise ValueError("Time length and data dimensions must be specified.")

        # 인코딩 및 디코딩 과정
        z = self.encode(inputs).sample()
        
        # decode 메서드에서 reshape할 때 None 값이 없도록 확인
        z_shape = tf.shape(z)
        if None in z_shape:
            raise ValueError("Shape of 'z' contains None values, which cannot be reshaped properly.")

        decoded_output = self.decode(z).sample()
