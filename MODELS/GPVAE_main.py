...
if gpus:
    try:
        ...
        print(f"GPUs are enabled: {[gpu.name for gpu in gpus]}")
    except RuntimeError as e:
        print(e)
else:
    print("No GPU found. Ensure the Docker container has access to GPUs.")
...
# from GPVAE import BandedJointEncoderGRU, GaussianDecoder, GP_VAE
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from utils.GPVAE import BandedJointEncoderGRU, GaussianDecoder, GP_VAE
...
with open('Train/config_GPVAE.yaml', 'r') as file:
...
if __name__ == "__main__":
    ...
    tf.compat.v1.keras.backend.set_epsilon(tf.compat.v1.Session(config=gpu_config))
    ...
    with summary_writer.as_default():
        for i, (x_seq, y_seq, m_seq) in enumerate(tf_x_train_miss.take(num_steps)):
            ...
            if i % print_interval == 0:
                ...
                global_gradient_norm = tf.linalg.global_norm(model.trainable_variables)
                ...
                print(f"Learning rate: {learning_rate.numpy():.6f} | Global gradient norm: {global_gradient_norm:.2f}")
                ...
                if val_loss_check > val_loss:
                    ...
                    model.encoder.net.save_weights(outdir + f'encoder_gpvae_val{val_loss.numpy():.4f}_valmse{val_mse.numpy():.4f}.weights.h5')
                    model.decoder.net.save_weights(outdir + f'decoder_gpvae_val{val_loss.numpy():.4f}_valmse{val_mse.numpy():.4f}.weights.h5')
                t0 = time.time()
