...
with open('config_GPVAE.yaml', 'r') as file:
        ...
        for i, (x_seq, y_seq, m_seq) in enumerate(tf_x_train_miss.take(num_steps)):
            ...
            if i % print_interval == 0:
                ...
                if val_loss_check > val_loss:
                    ...
                    model.encoder.net.save_weights(outdir + f'encoder_gpvae_val{val_loss.numpy()}_valmse{val_mse.numpy()}.hdf5')
                    model.decoder.net.save_weights(outdir + f'decoder_gpvae_val{val_loss.numpy()}_valmse{val_mse.numpy()}.hdf5')
